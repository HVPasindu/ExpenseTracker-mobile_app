...
const VerifyResetOtpScreen = ({navigation, route}) => {
  ...
  const handleVerifyResetOtp = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        'https://expense-tracker-backend-o3ow.onrender.com/users/verify-reset-otp',
        {
          email: email,
          otp: otp,
        },
  ...
  const handleResendResetOtp = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        'https://expense-tracker-backend-o3ow.onrender.com/users/resend-reset-otp',
        {
          email: email,
        },
      );

      setSuccessMessage(response.data.message || 'Reset OTP sent again');
    } catch (error) {
      setServerError(
        error.response?.data?.message || 'Cannot connect to server',
      );
        ...
        <View style={styles.card}>
          ...
          <TouchableOpacity
            ...
            {resendLoading ? (
              ...
              <Text style={styles.outlineButtonText}>RESEND RESET OTP</Text>
            )}
          </TouchableOpacity>
